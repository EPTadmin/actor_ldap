...
for (let a=0; a<lineChartData_thermo.datasets.map(x=>x.label).length;a++){
    data_set.push({label : lineChartData_thermo.datasets.map(x=>x.label)[a],data : lineChartData_thermo.datasets.map(x=>x.data)[a],backgroundColor : color[a]})
  }
